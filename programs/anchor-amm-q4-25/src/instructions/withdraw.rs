use anchor_lang::prelude::*;
use anchor_spl::{
    associated_token::AssociatedToken,
    token::{Burn, Mint, Token, TokenAccount, Transfer, burn, transfer}, token_2022::Token2022,
};
use constant_product_curve::ConstantProduct;

use crate::{errors::AmmError, state::Config};

#[derive(Accounts)]
pub struct Withdraw<'info> {
    #[account(mut)]
    pub user: Signer<'info>,

    #[account(
        mut,
        associated_token::mint = mint_x,
        associated_token::authority = config    
    )]
    pub vault_x: Account<'info, TokenAccount>,

    #[account(
        mut, 
        associated_token::mint = mint_y,
        associated_token::authority = config
    )]
    pub vault_y: Account<'info, TokenAccount>,

    #[account(
        mut, 
        associated_token::mint = mint_x,
        associated_token::authority = user
    )]
    pub user_x: Account<'info, TokenAccount>,

    #[account(
        mut,
        associated_token::mint = mint_y,
        associated_token::authority = user
    )]
    pub user_y: Account<'info, TokenAccount>,

    pub mint_x: Account<'info, Mint>,
    pub mint_y: Account<'info, Mint>,

    #[account(
        mut,
        seeds = [b"lp", config.key().as_ref()],
        bump = config.lp_bump
    )]
    pub mint_lp: Account<'info, Mint>,

    #[account(
        init_if_needed,
        payer = user,
        associated_token::mint = mint_lp,
        associated_token::authority = user
    )]
    pub user_lp: Account<'info, TokenAccount>,

    #[account(
        has_one = mint_x,
        has_one = mint_y,
        seeds = [b"config", config.seed.to_le_bytes().as_ref()],
        bump = config.config_bump
    )]
    pub config: Account<'info, Config>,

    pub token_program: Program<'info, Token>,
    pub system_program: Program<'info, System>,
    pub associated_token_program: Program<'info, AssociatedToken>
}

impl<'info> Withdraw<'info> {
    pub fn withdraw(
        &mut self,
        amount: u64, // Amount of LP tokens that the user wants to "burn"
        min_x: u64,  // Minimum amount of token X that the user wants to receive
        min_y: u64,  // Minimum amount of token Y that the user wants to receive
    ) -> Result<()> {
        require!(self.config.locked == false, AmmError::PoolLocked);
        require!(amount != 0, AmmError::InvalidAmount);
        
        //Great job done by the guy who made our life easier. This function basically gives us 
        //the resulting x and y tokens we can receive if we want to have a amount of lp tokens.
        //We can then check the result against the user requirements.
        let amounts = ConstantProduct::xy_withdraw_amounts_from_l(
            self.vault_x.amount,
            self.vault_y.amount,
            self.mint_lp.supply,
            amount,
            6,
        )
        .unwrap();
        let x = amounts.x;
        let y = amounts.y;

        require!(x >= min_x && y >= min_y, AmmError::LiquidityLessThanMinimum);

        // deposit token x
        self.withdraw_tokens(true, x)?;
        // deposit token y
        self.withdraw_tokens(false, y)?;
        // mint lp tokens
        self.burn_lp_tokens(amount)
    }

    pub fn withdraw_tokens(&self, is_x: bool, amount: u64) -> Result<()> {
        let (from, to) = match is_x {
            true => (
                self.vault_x.to_account_info(),
                self.user_x.to_account_info()
            ),
            false => (
                self.vault_y.to_account_info(),
                self.user_y.to_account_info(),
            ),
        };

        let ctx_accounts = Transfer {
            from,
            to,
            authority: self.user.to_account_info()
        };

        let ctx_program = self.token_program.to_account_info();

        let ctx_withdraw = CpiContext::new(ctx_program, ctx_accounts);

        transfer(ctx_withdraw, amount)
    }

    pub fn burn_lp_tokens(&self, amount: u64) -> Result<()> {
        require!(amount > 0, AmmError::InvalidAmount);
        let bump = self.config.lp_bump;
        let config_key = self.config.key();
        let seeds: &[&[&[u8]]] = &[&[
            b"lp".as_ref(),
            config_key.as_ref(),
            &[bump]
        ]];

        let cpi_program = self.token_program.to_account_info();

        let ctx_accounts = Burn {
            mint: self.mint_lp.to_account_info(),
            from: self.user_lp.to_account_info(),
            authority: self.user.to_account_info(),
        };

        let ctx_burn = CpiContext::new_with_signer(cpi_program, ctx_accounts, seeds);
        burn(ctx_burn, amount)
    }
}
